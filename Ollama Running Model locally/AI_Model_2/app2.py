...
class RAG_Agent:
    ...
    def run(self):
        ...
        qa_chain = (
            {
                "context": self.retriever | format_docs,  # Retrieve documents and format them
                "question": RunnablePassthrough()  # Pass the question directly
            }
            | self.prompt  # Apply the prompt
            | self.model   # Query the LLM
...
if __name__ == "__main__":
    agent = RAG_Agent()
    agent.set_model("llama3.1")  # Set the model to llama3.1 from Ollama
