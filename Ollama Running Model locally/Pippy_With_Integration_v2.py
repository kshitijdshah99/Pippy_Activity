...
class PippyActivity(ViewSourceActivity):
    ...
    def _fetch_ai_response(self, message):
        """
        Fetch the response from the FastAPI server.
        """
        url = "http://127.0.0.1:8000/query/"  # Ensure FastAPI server is running
        payload = {"question": message}
        headers = {"Content-Type": "application/json"}

        try:
            print(f"Sending POST request: {payload}")  # Debugging
            ...
            print(f"Error in _fetch_ai_response: {e}")
    ...
    def _ai_chatbot(self, button):
        ...
        for msg, msg_type in self.chat_messages:
            # Create a selectable text view for the message
            text_view = Gtk.TextView()
            text_view.set_wrap_mode(Gtk.WrapMode.WORD)  # Enable word wrapping
            text_view.set_editable(False)  # Disable editing
            text_view.set_cursor_visible(False)  # Hide the cursor
            text_view.get_style_context().add_class("user-message" if msg_type == "You" else "bot-message")

            # Add the message text to the text view
            buffer = text_view.get_buffer()
            buffer.set_text(f"{msg_type}: {msg}")
            ...
            # Pack the text view into the message box
            message_box.pack_start(text_view, False, False, 0)
        ...
        def _handle_message(button, text_entry, message_box):
            ...
            user_bubble = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            user_label = Gtk.Label(label=message)
            user_label.set_xalign(0)
            user_label.set_line_wrap(True)
            user_label.set_max_width_chars(100)
            user_label.set_selectable(True)
            user_label.set_can_focus(False)
            user_label.get_style_context().add_class("user-message")
            user_bubble.pack_start(user_label, False, False, 0)
            message_box.pack_start(user_bubble, False, False, 10)
            message_box.show_all()
            ...
            # Fetch AI response
            ai_response = self._fetch_ai_response(message)

            # Add bot response as a bubble
            bot_bubble = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=6)
            bot_label = Gtk.Label(label=ai_response)
            bot_label.set_xalign(0)
            bot_label.set_line_wrap(True)
            bot_label.set_max_width_chars(100)
            bot_label.set_selectable(True)
            bot_label.set_can_focus(False)
            bot_label.get_style_context().add_class("bot-message")
            bot_bubble.pack_start(bot_label, False, False, 0)
            message_box.pack_start(bot_bubble, False, False, 10)
            message_box.show_all()

            # Save the bot response in chat history
            self.chat_messages.append((ai_response, "Chatbot"))

            # Scroll to the latest message
            adj = message_box.get_parent().get_vadjustment()
            GLib.idle_add(adj.set_value, adj.get_upper() - adj.get_page_size())

        # Attach the send button to the message handler
